...
from fwbackups.const import LOGLOC, MSWINDOWS, SETLOC
import fwbackups
...
class TextViewConsole:
    ...
    def history(self):
        ...
        stream.close()
        if text.strip() != '':
            self.write_line('-- %s --%s%s' % (_('Previous log messages'),
                                              os.linesep, os.linesep), self.style_ps2)
            for i in text.split('\n'):
                self.write_log_line(i)
        else:
            self.write_line('-- %s --%s' % (_('No previous log messages to display'),
                                            os.linesep), self.style_ps2)
        self.write_line('-- %s --%s%s' % (_('Current session\'s log messages'),
                                          os.linesep, os.linesep), self.style_ps2)
    ...
    def history(self):
        ...
        if text.strip() != '':
            self.console.write_line(
                '-- %s --%s%s' % (_('Previous log messages'), os.linesep, os.linesep), self.console.style_ps2)
            for i in text.split(os.linesep):
                errType = i.split(' ')
                if len(errType) >= 5:
                    if errType[4] == _('CRITICAL') or errType[4] == _('ERROR'):
                        self.console.write_line(
                            '%s%s' % (i, os.linesep), self.console.style_err)
                    elif errType[4] == _('WARNING'):
                        self.console.write_line(
                            '%s%s' % (i, os.linesep), self.console.style_warn)
                    else:
                        self.console.write_line(
                            '%s%s' % (i, os.linesep), self.console.style_ps1)
                else:
                    self.console.write_line(
                        '%s%s' % (i, os.linesep), self.console.style_ps1)
        else:
...
class StatusBar:
    ...
    def newmessage(self, message, seconds=3):
      ...
      """
        ...
        self.message_timer = gobject.timeout_add(
            seconds*1000, self.message_timeout)
...
class ConfirmDia(GenericDia):
    ...
    def __init__(self, dialog, parent, labelWidget, primaryText, secondaryText, dontShowMe=False):
        ...
        labelWidget.set_text('<span size="larger" weight="bold">%s</span>\n\n%s' %
                             (escape(primaryText), escape(secondaryText)))
...
class PathView(View):
    ...
    def __init__(self, treeview, statusbar, ui, parent):
        ...
        self.treeview.drag_dest_set(
            gtk.DEST_DEFAULT_ALL, target, gtk.gdk.ACTION_COPY)
        ...
        def get_local_path(uri):
            ...
            if uri[0] == '/':
                ...
                if i == 2:
                    if MSWINDOWS:
                        # ///path - from the removal of file:///
                        return unescape(uri[3:])
                    else:
                        # ///path - from the removal of file:// as we need / (is root)
                        return unescape(uri[2:])
                remote_host = uri[2:i]
                if remote_host == gethostname():
    ...
    def addFolder(self):
        ...
        fileDialog = PathDia(self.ui.path_dia, _(
            'Choose folder(s)'), self.parent, gtk.FILE_CHOOSER_ACTION_SELECT_FOLDER, multiple=True)
        response = fileDialog.run()
        if response == gtk.RESPONSE_OK:
            paths = [path.decode('utf-8')
                     for path in fileDialog.get_filenames()]
        ...
        for file in files:
            if file.endswith('.conf'):
                ...
                self.logger.logmsg('WARNING', _(
                    'Refusing to parse file `%s\': configuration files must end in `.conf\'') % file)
