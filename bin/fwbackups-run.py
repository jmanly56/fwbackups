...
from fwbackups.const import INSTALL_DIR, MSWINDOWS, SETLOC
...
if __name__ == "__main__":
    ...
    try:
        ...
        (opts, rest_args) = getopt.gnu_getopt(
            sys.argv[1:], "hvlf", avalableOptions)
    ...
    try:  # Pycron passes arguments as UTF-8 encoded bytestrings
        ...
        sets = [set for set in sys.argv[1:]]
    for i in opts:
        for ii in i:
            try:
                sets.remove(ii)
    ...
    if verbose is True or int(prefs.get('Preferences', 'AlwaysShowDebug')) == 1:
    ...
    for setPath in sets:
        ...
        if not os.path.exists(encode(setPath)):
            logger.logmsg("ERROR", _(
                "The set configuration for '%s' was not found - skipping." % setPath))
            continue
        try:
            backupHandle = backup.SetBackupOperation(
            ...
            logger.logmsg('WARNING', _(
                "An error occurred while initializing the backup operation!"))
        ...
        # thread returned
        logger.logmsg('DEBUG', _('Thread returned with retval %s' %
                                 backupThread.retval))
