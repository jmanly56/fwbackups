...
class Common:
            ...
            for process in self.pids:
                try:
                    self.logger.logmsg('DEBUG', _(
                        'Stopping process with ID %s') % process)
                    fwbackups.kill(process, 9)
                except Exception as error:
                    self.logger.logmsg('WARNING', _(
                        'Could not stop process {0}: {1}'.format(process, error)))
                    continue
        self.toCancel = True
    ...
    def prepareDestinationFolder(self, folder, createNonExistant=True):
        ...
        if os.path.exists(encode(folder)):
            self.logger.logmsg('DEBUG', _(
                'Destination folder `%s\' exists') % folder)
        # folder doesn't exist; try to create if specified
        elif createNonExistant:
            try:
                os.mkdir(encode(folder), 0o755)
                self.logger.logmsg('DEBUG', _(
                    "Created destination folder `%s'") % folder)
            except OSError as error:
                self.logger.logmsg('ERROR', _(
                    "The destination folder `%(a)s' could not be created: %(b)s") % {'a': folder, 'b': error})
                return False
        # make sure the folder is writable
        if not fwbackups.CheckPerms(folder):
            self.logger.logmsg('WARNING', _(
                "You do not have read and write permissions on the destination `%s' - if you backed up system files, this operation may fail.") % folder)  # noqa: E501
        return True
