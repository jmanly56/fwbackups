...
def connect(host, username, password, port=22, timeout=120):
    ...
    except Exception:  # < v1.7.2 - no compression support
        pass
...
def iglob(pattern, folder):
    if not glob.has_magic(pattern):  # nothing to expand
        # make relative to folder
        pattern2 = os.path.normpath(folder+os.sep+pattern)
    ...
    for item in os.listdir(src):
        ...
        try:
            ...
            if symlinks and os.path.islink(src_abs):
                ...
                putFolder(sftp, src_abs, os.path.dirname(
                    dst_abs), symlinks, excludes)
