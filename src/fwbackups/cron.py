...
from fwbackups.const import CRON_SIGNATURE, MSWINDOWS
...
class crontabLine(rawCrontabLine):
    ...
    def __init__(self, minute, hour, dayofmonth, month, dayofweek, command, end_comment=''):
        ...
        self.set_all_fields(minute, hour, dayofmonth, month,
                            dayofweek, command, end_comment)
    ...
    def is_parsable(self):
        ...
        for field in [self.__minute, self.__hour, self.__dayofmonth, self.__month, self.__dayofweek]:
            if re.compile(r"^[0-9/\-,*]*$").search(field) is None:
                return False
        return True
    ...
    def get_all_fields(self):
        ...
        return (self.__minute, self.__hour, self.__dayofmonth, self.__month,
    ...
    def validate(self):
        ...
        if not self.is_parsable() and not self.is_comment_or_whitespace():
            raise ValueError(
                "Entry is not parsable - cannot generate entry text")
...
def getPyCrontab():
    ...
    # No cron file found!
        ...
        raise CronError(
            _("Could not locate the pycron or the sample pycron configuration in %s" % pycronLoc))
    # Read in the configuration file we found
    pycrontab = os.path.join(
        pycronLoc, pycronConf.get('pycron', 'crontab_filename'))
    if not os.path.exists(pycrontab):
        try:
            ...
            raise CronError(
                _("Could not locate the pycron or the sample pycron crontab file in %s" % pycronLoc))
    return pycrontab
...
def remove():
    ...
    if MSWINDOWS:
        ...
        sub, stdout, stderr = execute(
            ['crontab', '-r'], stdoutfd=subprocess.PIPE)
...
def clean_fwbackups_entries():
    ...
    for line in crontabLines:
        if line.is_comment_or_whitespace():
            ...
            match = re.match(
                r"^([^\s]+)\s+([^\s]+)\s+([^\s]+)\s+([^\s]+)\s+([^\s]+)\s+(.+?)\s*(#.*)?$", rawtext.strip())
            if match is None:
                cleanedLines.append(line)
                continue
            try:
