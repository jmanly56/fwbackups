...
class RestoreOperation(operations.Common):
    def __init__(self, restorePath, logger=None):
        ...
        # workaround so prepareDestinationFolder doesn't complain
        self.options['Engine'] = 'null'
    ...
    def start(self):
        ...
        if self.options['RemoteSource']:  # check if server settings are OK
            ...
            while thread.retval is not None:
                time.sleep(0.1)
            # Check for errors, if any
            import paramiko
            import socket
            if thread.retval is not True:
                pass
        ...
        if self.options['RemoteSource']:
            ...
            try:
                ...
                client, sftpClient = sftp.connect(
                    self.options['RemoteHost'], self.options['RemoteUsername'],
                    self.options['RemotePassword'], self.options['RemotePort'])
                sftp.receive(sftpClient, self.options['RemoteSource'],
                ...
                self.logger.logmsg('ERROR', _(
                    'Could not receive file from server: %s' % error))
                wasErrors = True
        ...
        try:
            if self.options['SourceType'] == 'set':  # we don't know the type
                if os.path.isfile(encode(self.options['Source'])):
                    ...
                    fh.close()
                # we are dealing with rsync
                elif os.path.isdir(encode(self.options['Source'])):
                    shutil_modded.copytree(
                        encode(self.options['Source']), encode(self.options['Destination']))
                else:  # oops, something is up
                    self.logger.logmsg('ERROR', _(
                        'Source `%s\' is not a file or folder!' % self.options['Source']))
                    return False
                ...
                if os.path.isfile(encode(self.options['Source'])):
                    ...
                    self.logger.logmsg('ERROR', _(
                        'Source `%s\' is not an archive!' % self.options['Source']))
                    return False
            else:  # folders
                if not os.path.isdir(encode(self.options['Source'])):
                    self.logger.logmsg('ERROR', _(
                        'Source `%s\' is not a folder' % self.options['Source']))
                    return False
                if not self.prepareDestinationFolder(self.options['Destination']):
                    return False
                shutil_modded.copytree(
            ...
            if self.options['SourceType'] == 'remote archive (SSH)' or \
                    (self.options['SourceType'] == 'set' and self.options['RemoteSource']):
