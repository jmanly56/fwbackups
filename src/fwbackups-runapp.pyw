...
from fwbackups.const import INSTALL_DIR, USER, MSWINDOWS, PREFSLOC, SETLOC, UID, LOGLOC, DARWIN, LINUX, USERHOME
from fwbackups.i18n import _

if sys.platform.startswith('win'):
    os.environ["PATH"] += ";%s" % os.path.join(INSTALL_DIR, "gtkfiles", "bin")
    os.environ["PATH"] += ";%s" % os.path.join(
        INSTALL_DIR, "gtkfiles", "addon")
...
def reportBug(etype=None, evalue=None, tb=None):
    """Report a bug dialog"""
    import traceback
    c = interface.Controller(os.path.join(
        INSTALL_DIR, 'BugReport.glade'), 'bugreport')
    if not etype and not evalue and not tb:
        (etype, evalue, tb) = sys.exc_info()
    tracebackText = ''.join(traceback.format_exception(etype, evalue, tb))
    reportWindow = widgets.bugReport(
        c.ui.bugreport, c.ui.bugreportTextview, None, tracebackText)
    ...
    if response == gtk.RESPONSE_OK:
        ...
        if fwbackups.CheckPerms(filename):
            import datetime
            fh = open(filename, 'w')
            fh.write(_('fwbackups bug report written saved at %s') %
                     datetime.datetime.today().strftime('%I:%M %p on %Y-%m-%d\n'))
...
try:
    ...
    raise fwbackups.fwbackupsError(
        _('Please install paramiko (python-paramiko)'))
...
class fwbackupsApp(interface.Controller):
    ...
    def __init__(self, verbose=0, minimized=False):
        ...
        interface.Controller.__init__(self, os.path.join(
            INSTALL_DIR, 'fwbackups.glade'), 'main')
    ...
    def _setDefaults(self):
        ...
        # Default Labels...
        set_text_markup(self.ui.aboutVersionLabel,
                        '<span size="xx-large" weight="bold">fwbackups %s</span>' % fwbackups.__version__)
        self.setStatus(_('Idle'))
        self.main2IconviewSetup()
        self.main3TabSetup()
        self.restoreSetup()
        # Default progressbars...
        self.main2BackupProgress.progressbar.set_text(
            _('Click \'Backup Set Now\' to begin'))
        self.main3BackupProgress.progressbar.set_text(
            _('Click \'Start Backup\' to begin'))
        self.ui.restore2RestorationProgress.set_text(
            _('Click \'Start Restore\' to begin'))
        # refresh main3
        self.main3Refresh()
    ...
    def updateSplash(self, fraction, text):
        ...
        set_text_markup(self.ui.label444, _(
            '<span size="small">%s%% complete: %s</span>' % (percent, text)))
        while gtk.events_pending():
            gtk.main_iteration()
        time.sleep(0.1)
    ...
    def runSetup(self, widget=None, event=None, minimized=False):
        ...
        try:
            ...
            response = self.displayConfirm(self.ui.splash, _("Preferences could not be read"), _(
                "The preferences file may be corrupted and could not be read by fwbackups. " +
                "Would you like to initialize a new one from the default values? If not, fwbackups will exit."))
        ...
        try:
            ...
            self._checkLogSize()
            self.logconsole = widgets.TextViewConsole(
                self.ui.LogViewerTextview)
        ...
        try:
            ...
            self.logger.logmsg('DEBUG', _(
                'pynotify was not found. Notifcations will not be displayed' +
                ' unless notify-python/pynotify is installed.'))
        ...
        self.ExportView1 = widgets.ExportView(
            self.ui.ExportTreeview, self.statusbar, self.ui)
        ...
        if os.path.exists('/etc/crontab') and fwbackups.CheckPermsRead('/etc/crontab'):
            ...
            if re.search('.*fwbackups.*', crontab):
                if int(prefs.get('Preferences', 'DontShowMe_OldVerWarn')) == 0:
                    dontShowMe = self.displayInfo(self.ui.main, _(
                        "Previous installation of fwbackups 1.42.x or older detected"), _(
                            "Because of certain major changes in fwbackups, your configuration must be imported manually. To do this, run fwbackups as the root user and select <i>File > Import</i> from the menu to import your old configuration.\nIf this action is not completed, the backups from version 1.42.x <b>will not run automatically</b>.\n<i>Tip: The 1.42.x configuration was stored in /etc/fwbackups</i>."), dontShowMe=True)  # noqa: E501
                    if dontShowMe:
            ...
            if UID == 0:
                try:
                    ...
                    self.displayInfo(self.ui.main, _("A backup of /etc/crontab could not be created"), _(
                        "You may wish to edit /etc/crontab manually and remove any fwbackups-related lines."))
                    self.logger.logmsg('ERROR', _(
                        "Could not create a backup of /etc/crontab; refusing to clean up."))
                try:
                    if re.search('.*fwbackups.*', crontab):
                        self.logger.logmsg(
                            'INFO', _('Cleaning up /etc/crontab from previous installation of fwbackups'))
                    ...
                    message = _(
                            "fwbackups was unable to clean up the system crontab: %s. You may wish to edit /etc/crontab manually and remove any fwbackups-related lines.") % error  # noqa: E501
                    self.displayInfo(self.ui.main, _(
                        "Cleanup of the system crontab failed"), message)
    ...
    def _checkLogSize(self):
        ...
        if statsize >= 1048577:
            ...
            response = self.displayConfirm(self.ui.splash, _("Would you like to clean up the log file?"), _(
                "The log file is becoming large (%s). Would you like to clear it? This will permanently remove all" +
                " entries from the log.") % size)
            if response == gtk.RESPONSE_YES:
                logfh = open(LOGLOC, 'w')
                logfh.write('')
                logfh.close()
    ...
    def _setupTrayIcon(self):
        ...
        self.ui.display_notifications1.set_active(
            prefs.getboolean('Preferences', 'ShowNotifications'))
        self.ui.minimize_to_tray_on_close1.set_active(
            prefs.getboolean('Preferences', 'MinimizeTrayClose'))
    ...
    def _Popup(self, status, button, time):
        ...
        # --
        def menu_pos(menu):
            width, height, orientation = gtk.status_icon_position_menu(
                menu, self.trayicon)
    ...
    def displayInfo(self, parent, primaryText, secondaryText, dontShowMe=False):
        ...
        dialog = widgets.InfoDia(
            self.ui.info_dia, parent, self.ui.infoDiaLabel, primaryText, secondaryText, dontShowMe)
        dialog.run()
        if dontShowMe:
    ...
    def displayError(self, parent, primaryText, secondaryText):
        """Wrapper for displaying the confirm dialog"""
        dialog = widgets.ErrorDia(
            self.ui.error_dia, parent, self.ui.errorDiaLabel, primaryText, secondaryText)
        dialog.run()
        dialog.destroy()
    ...
    def displayWarning(self, parent, primaryText, secondaryText, dontShowMe=False):
        ...
        dialog = widgets.WarningDia(
            self.ui.warning_dia, parent, self.ui.warningDiaLabel, primaryText, secondaryText, dontShowMe)
        dialog.run()
        if dontShowMe:
            showMe = dontShowMe.get_active()
            dialog.destroy()
    ...
    def displayConfirm(self, parent, primaryText, secondaryText, dontShowMe=False):
        ...
        dialog = widgets.ConfirmDia(
            self.ui.confirm_dia, parent, self.ui.confirmDiaLabel, primaryText, secondaryText, dontShowMe)
        response = dialog.run()
        if dontShowMe:
            showMe = dontShowMe.get_active()
            dialog.destroy()
    ...
    def on_minimize_to_tray_on_close1_activate(self, widget, event=None):
        ...
        self.ui.preferencesMinimizeTrayCloseCheck.set_active(
            self.ui.minimize_to_tray_on_close1.get_active())

    def on_display_notifications1_activate(self, widget, event=None):
        """Set display notifications from tray icon"""
        self.ui.preferencesShowNotificationsCheck.set_active(
            self.ui.display_notifications1.get_active())
    ...
    def trayNotify(self, summary, body, timeout=-1):
        ...
        try:
            ...
            # icon
            pix = self.ui.main.render_icon(
                gtk.STOCK_COPY, gtk.ICON_SIZE_DIALOG)
            ...
            self.logger.logmsg('DEBUG', _(
                'Could not create notification: %s' % error))
    ...
    def help(self):
        ...
        import webbrowser
        webbrowser.open_new(
            'http://downloads.diffingo.com/fwbackups/docs/%s-html' % fwbackups.__version__)
        ...
        for file in files:
            if file.endswith('.conf') and file != 'temporary_config.conf':
                ...
                try:
                    ...
                    self.displayError(self.ui.main,
                                      _("Could not parse the configuration for set '%s'") % os.path.splitext(file)[0],
                                      _("The set configuration file '%s' failed to validate, so it may not be" +
                                          " properly scheduled. Other sets are unaffected by this error." % file))
                    continue
                ...
                try:
                    ...
                    if not crontabLine.is_parsable():
                        raise cron.CronError(
                            _("Internal error: generated entry for set '%s' was not parsable." +
                                "Please submit a bug report.") % setName)
                ...
                self.logger.logmsg('DEBUG', _(
                    "Saving set `%s' to the crontab") % setName)
        try:
            ...
            self.logger.logmsg('WARNING', _(
                "Unable to write new crontab: %s" % tracebackText))
            self.displayError(self.ui.main, _("Unable to save backup schedule to crontab"), _(
                "There was an error saving the new backup schedule. A backup will be restored, however the changes made to the backup schedule may not have been correctly saved.\n\nIf you see this message, please report a bug against fwbackups."))  # noqa: E501
            # Restore backup
            try:
                cron.write(originalCronLines)
                self.logger.logmsg('INFO', _(
                    "A backup of the crontab was restored; the latest changes to" +
                    " the backup schedule not have been saved."))
            except Exception:
                self.logger.logmsg('ERROR', _(
                    "A backup of the crontab could not be restored"))
    ...
    def main_close(self, widget=None, event=None):
        ...
        if self.operationInProgress:
            response = self.displayConfirm(self.ui.main,
                                           ...
                                           _("An operation is currently in progress. Would you like to cancel " +
                                               "it and quit anyways?"))
            if response == gtk.RESPONSE_YES:
                self.statusbar.newmessage(
                    _('Please wait... Canceling operations'), 10)
                try:
        ...
        if len(threading.enumerate()) > 1:
            self.logger.logmsg(
                'INFO', _('fwbackups detected more than one alive threads!'))
        for thread in threading.enumerate()[1:]:
            if type(thread) == paramiko.Transport:
                # Thread is a Paramiko transport -- close it down.
                thread.close()
                self.logger.logmsg(
                    'INFO', _('Closed dead Paramiko thread: %s') % str(thread))
            else:
                # FIXME: Figure out how to kill other thread types. For now, just log.
                self.logger.logmsg('CRITICAL', _(
                    'Could not close thread %s!') % str(thread))
                self.logger.logmsg('CRITICAL', _(
                    'Please submit a bug report and include this message.'))
        ...
        try:
            ...
            self.displayError(self.ui.backupset, _("Could not save backup schedule"), _(
                "fwbackups was unable to save the backup scheduling information to the crontab. If you are using" +
                " manual times entry in one of your sets, please verify that all five" +
                " fields are using the correct syntax."))
    ...
    def testConnection(self, parent, progress, host, username, password, port, path):
        ...
        thread = fwbackups.runFuncAsThread(
            sftp.testConnection, host, username, password, port, path)
        ...
        progress.set_text('')
        self.logger.logmsg('DEBUG', _(
            'testConnection(): Thread returning with retval %s' % str(thread.retval)))
        if thread.retval is True:
            self.displayInfo(parent, _("Success!"), _(
                "fwbackups was able to connect and authenticate on '%s'.") % host)
        elif type(thread.exception) == IOError:
            self.displayInfo(parent, _("Remote destination is not usable"), _(
                "The folder '%(a)s' on '%(b)s' either does not exist or it is not a folder. Please verify your settings and try again.") % {'a': path, 'b': host})  # noqa:E501
        elif type(thread.exception) == paramiko.AuthenticationException:
            self.displayInfo(parent, _("Authentication failed"), _(
                "A connection to '%s' was established, but authentication failed. Please verify the username and password and try again.") % host)  # noqa:E501
        elif type(thread.exception) == socket.gaierror or type(thread.exception) == socket.error:
            self.displayInfo(parent, _("A connection to '%s' could not be established") % host, _(
                "Error %(a)s: %(b)s") % {'a': type(thread.exception), 'b': str(thread.exception)})
        elif type(thread.exception) == socket.timeout:
            self.displayInfo(parent, _("The server '%s' is not responding") % host, _(
                "Please verify your settings and try again."))
        elif type(thread.exception) == paramiko.SSHException:
            self.displayInfo(parent, _("A connection to '%s' could not be established") % host, _(
                "Error: %s. Please verify your settings and try again.") % str(thread.exception))
        else:
    ...
    def on_import_sets1_activate(self, widget):
        ...
        fileDialog = widgets.PathDia(self.ui.path_dia, _('Select file(s)'), self.ui.main,
                                     gtk.FILE_CHOOSER_ACTION_OPEN, ffilter=[
                                         '*.conf', _('Configuration files (*.conf)')],
                                     multiple=False)
        response = fileDialog.run()
        if response == gtk.RESPONSE_OK:
            oldSetPaths = [path.decode('utf-8')
            ...
            for oldSetPath in oldSetPaths:
                ...
                if "General" not in oldSetConf.sections():
                    ...
                    if oldSetConf.get("prefs", "autotar"):
                        paths.append("/etc")
                    options = {}
                    options["PkgListsToFile"] = int(
                        oldSetConf.get('prefs', 'autorpm'))
                    options["OldToKeep"] = int(
                        oldSetConf.get('prefs', 'tokeep'))
                    if UID == 0:  # this can only be done if the user is root
                        options["DiskInfoToFile"] = int(
                            oldSetConf.get('prefs', 'autodiskinfo'))
    ...
    def _exportSet(self, model, path, iter, dest):
       ...
       """
        if model.get_value(iter, 0):
            ...
            self.logger.logmsg(
                'INFO', _('Exported set `%(a)s\' to `%(b)s\'' % {'a': setName, 'b': dest}))

    def on_export_sets1_activate(self, widget):
        """Export Sets entry in menu"""
        self.ExportView1.refresh()
        exportDialog = widgets.GenericDia(
            self.ui.export_dia, _('Export Sets'), self.ui.main)
        ...
        while runLoop:
            if response == gtk.RESPONSE_HELP:
                ...
                self.ExportView1.liststore.foreach(
                    self._exportSet, destination)
                self.displayInfo(self.ui.export_dia, _('Sets exported'), _(
                    'The selected sets were exported successfully.'))
                break
    ...
    def on_duplicate_set1_activate(self, widget):
        ...
        try:
            ...
            self.displayWarning(self.ui.main, _('No set has been selected'), _(
                'You must select a set to duplicate.'))
            return
        config.BackupSetConf(setPath)
        newSetName = setName
        while True:
            newSetName += _("_copy")
 ...
 """
    ...
    def on_preferences1_activate(self, widget, event=None):
        ...
        self.ui.preferencesShowTrayIconCheck.set_active(
            prefs.getboolean('Preferences', 'ShowTrayIcon'))
        self.ui.preferencesMinimizeTrayCloseCheck.set_active(
            prefs.getboolean('Preferences', 'MinimizeTrayClose'))
        self.ui.preferencesStartMinimizedCheck.set_active(
            prefs.getboolean('Preferences', 'StartMinimized'))
        self.ui.preferencesShowNotificationsCheck.set_active(
            prefs.getboolean('Preferences', 'ShowNotifications'))
        if MSWINDOWS:
            import _winreg
            k = _winreg.CreateKey(
                _winreg.HKEY_CURRENT_USER, "Software\\Microsoft\\Windows\\CurrentVersion\\Run")
            try:
            ...
            exists = os.path.exists(
                '%s/.config/autostart/fwbackups-autostart.desktop' % USERHOME)
        elif DARWIN:  # FIXME: Figure out autostart on OS X
            exists = False
        self.ui.preferencesSessionStartupCheck.set_active(exists)
        self.ui.preferencesAlwaysShowDebugCheck.set_active(
            prefs.getboolean('Preferences', 'AlwaysShowDebug'))
        self.ui.preferencesMinimizeTrayCloseCheck.set_active(
            prefs.getboolean('Preferences', 'MinimizeTrayClose'))
        ...
        if (tempDir):
            ...
            self.ui.preferencesCustomizeTempDirBrowseButton.set_sensitive(
                False)

        if MSWINDOWS:
            self.ui.preferencesPycronEntry.set_text(
                prefs.get('Preferences', 'pycronLoc'))
    ...
    def on_check_for_updates1_activate(self, widget):
        ...
        import webbrowser
        webbrowser.open_new(
            "http://www.diffingo.com/update.php?product=fwbackups&version=%s" % fwbackups.__version__)
    ...
    def on_preferencesSessionStartupCheck_clicked(self, widget):
        ...
        if self.ui.preferencesSessionStartupCheck.get_active():  # add
            if MSWINDOWS:
                ...
                k = _winreg.CreateKey(
                    _winreg.HKEY_CURRENT_USER, "Software\\Microsoft\\Windows\\CurrentVersion\\Run")
                _winreg.SetValueEx(k, 'fwbackups', 0, _winreg.REG_SZ, '"%s" --start-minimized' %
                                   os.path.join(INSTALL_DIR, 'fwbackups-runapp.pyw'))
                _winreg.CloseKey(k)
            else:
                shutil_modded.copy('%s/fwbackups-autostart.desktop' %
                                   INSTALL_DIR, '%s/.config/autostart' % USERHOME)
        else:  # remove
            if MSWINDOWS:
                import _winreg
                try:
                    ...
                    self.logger.logmsg('DEBUG', _(
                        'Could not remove fwbackup\'s session autostart regkey: %s' % error))
            else:
                try:
                    os.remove(
                        '%s/.config/autostart/fwbackups-autostart.desktop' % USERHOME)
                    ...
                    self.logger.logmsg('DEBUG', _(
                        'Could not remove fwbackup\'s session autostart file: %s' % error))
    ...
    def on_preferencesResetDontShowMeButton_clicked(self, widget, event=None):
        ...
        for option in prefs.getPreferences().keys():
            if option.lower().startswith('dontshowme_'):
                ...
                self.logger.logmsg('DEBUG', _(
                    'Resetting preference \'%(a)s\' to 0' % {'a': option}))
    ...
    def on_preferencesCloseButton_clicked(self, widget):
        ...
        if MSWINDOWS:
            ...
            if not pycronLoc:
                ...
                return
            if not os.path.exists('%s/pycron.cfg' % pycronLoc) and not os.path.exists('%s/pycron.cfg.sample' % pycronLoc):  # noqa: E501
                self.displayInfo(self.ui.preferences,
    ...
    def on_backupset2LocalFolderEntry_changed(self, widget):
        ...
        self._checkDestPerms(widget.get_text().decode(
            'utf-8'), self.ui.backupset2FolderPermissionImage)
    ...
    def on_backupset2DestinationTypeCombobox_changed(self, widget):
        ...
        tables = [self.ui.backupset2DestinationTypeTable0,
                  self.ui.backupset2DestinationTypeTable1]
        for i in tables:
    ...
    def on_backupset2TestSettingsButton_clicked(self, widget):
        ...
        if not (host and username and port and folder):
            self.displayInfo(self.ui.backupset, _("Missing information"), _(
                "Please complete the host, username, folder and port fields."))
            return False
        self.testConnection(self.ui.backupset, self.backupset2TestSettingsProgress,
                            host, username, password, port, folder)
    ...
    def on_backupset3EasyConfigExpander_activate(self, widget):
        ...
        self.ui.backupset3EasyConfigTable.set_sensitive(
            not self.ui.backupset3EasyConfigTable.get_property('sensitive'))
        self.ui.backupset3ManualConfigTable.set_sensitive(
            not self.ui.backupset3ManualConfigTable.get_property('sensitive'))
        time.sleep(0.2)
        self.ui.backupset3ManualConfigExpander.set_expanded(
            not self.ui.backupset3ManualConfigExpander.get_expanded())
    ...
    def on_backupset3ManualConfigExpander_activate(self, widget):
        ...
        self.ui.backupset3ManualConfigTable.set_sensitive(
            not self.ui.backupset3ManualConfigTable.get_property('sensitive'))
        time.sleep(0.2)
        self.ui.backupset3EasyConfigExpander.set_expanded(
            not self.ui.backupset3EasyConfigExpander.get_expanded())
    ...
    def on_main2NewSetButton_clicked(self, widget):
        ...
        self._toggleLocked(
            True, [self.ui.BackupSetsRadioTool, self.ui.backup_sets1, self.ui.backupset])
    ...
    def on_main2RemoveSetButton_clicked(self, widget):
        ...
        try:
            ...
            self.displayWarning(self.ui.main, _('No set has been selected'), _(
                'You must select a set to remove.'))
            return
        response = self.displayConfirm(self.ui.main, _("Delete backup set '%s'?") % setName, _(
            "This action will permanently delete the backup set configuration. " +
            "This action does not delete any existing backups or files."))
        if response == gtk.RESPONSE_NO:
            ...
            try:
                ...
                message = _('An error occured while removing set `%(a)s\':\n%(b)s' % {
                            'a': setName, 'b': error})
                self.displayError(self.ui.main, _(
                    "Could not remove backup set '%s'") % setName, message)
                self.logger.logmsg('ERROR', message)
                self.main2IconviewRefresh()
                return
            try:
                ...
                message = _('An error occured while removing set `%(a)s\':\n%(b)s' % {
                            'a': setName, 'b': error})
                self.displayError(self.ui.main, _(
                    "Could not remove the automated backup schedule for set '%s'"), message)
                self.logger.logmsg('ERROR', message)
                self.main2IconviewRefresh()
                return
    ...
    def on_main2Iconview_selection_changed(self, widget):
        ...
        try:
            ...
            self.displayInfo(self.ui.main, _("Unexpected error"), _(
                "An unexpected error occured while attempting to obtain the selected set:\n%s" % error))
            self.main2IconviewRefresh()
            selected = False
    ...
    def on_main2RestoreSetButton_clicked(self, widget):
        ...
        try:
            ...
            self.displayWarning(self.ui.main, _('No set has been selected'), _(
                'You must select a set to restore.'))
            return
        ...
        if currentPage == 1:  # switching 3rd > 4th (last configurable page)
            ...
            if active == 0:  # local
                if not self.ui.main3LocalFolderEntry.get_text():
                    self.displayInfo(self.ui.main, _('Missing information'), _(
                        'Please fill in the Folder field.'))
                    return
                ...
                if not self.ui.main3HostEntry.get_text() or not self.ui.main3UsernameEntry.get_text()\
                    ...
                    self.displayInfo(self.ui.main, _('Missing information'), _(
                        'Please fill in the Host, Username, Port and Folder fields.'))
                    return
    ...
    def on_main3DestinationTypeCombobox_changed(self, widget):
        ...
        tables = [self.ui.main3DestinationTypeTable0,
                  self.ui.main3DestinationTypeTable1]
        for i in tables:
            i.set_sensitive(False)
        tables[active].set_sensitive(True)
    ...
    def on_main3LocalFolderEntry_changed(self, widget):
        ...
        self._checkDestPerms(widget.get_text().decode(
            'utf-8'), self.ui.main3FolderPermissionImage)
    ...
    def on_main3TestSettingsButton_clicked(self, widget):
        ...
        if not (host and username and port and folder):
            self.displayInfo(self.ui.main, _('Missing information'), _(
                'Please complete all of the host, username, folder and port fields.'))
            return False
        self.testConnection(self.ui.main, self.main3TestSettingsProgress,
                            host, username, password, port, folder)
    ...
    def updateLogViewer(self, severity, message):
        ...
        gobject.idle_add(self.logconsole.write_log_line,
    ...
    def on_LogViewerSaveToFileButton_clicked(self, widget):
        ...
        if saveFilename:
            ...
            saveto.close()
            self.logger.logmsg('DEBUG', _(
                "Log saved to file `%s'") % saveFilename)
    ...
    def _populateDates(self, setName):
        ...
        if setConfig.get('Options', 'DestinationType') == 'remote (ssh)':
            ...
            self.ui.restore1SetDateCombobox.set_active(0)
            thread = fwbackups.runFuncAsThread(
                self.getRemoteBackupDates, setConfig)
            ...
            if type(thread.retval) == list:
                ...
                self.displayInfo(self.ui.restore, _(
                    'Unexpected error'), thread.traceback)
            ...
            try:
                ...
                self.logger.logmsg("WARNING", _("Error obtaining file listing in destination %(a)s:\n%(b)s") % {
                                   'a': destination, 'b': error})
                self.displayError(self.ui.restore, _("Could not get a list of old backups for set '%s'" % setName), _(
                    "If the destination is on removable media such as an external hard disk, please attach it and try again."))
                return
    ...
    def on_restore1TestSettingsButton_clicked(self, widget):
        ...
        if self.ui.restore1SourceTypeCombobox.get_active() == 3:
            self.testConnection(
                self.ui.restore, self.restore1TestSettingsProgress, host, username, password, port, path)
        else:
            self.testConnection(
                self.ui.restore, self.restore1TestSettingsProgress, host, username, password, port, path)
    ...
    def saveRestoreConfiguration(self, restoreConf, setConfig=None):
        ...
        options["Destination"] = self.ui.restore1DestinationEntry.get_text().decode(
            'utf-8')
        ...
        if active == 0:  # set
            sourceType = 'set'
            date = ' - '.join(
                self.ui.restore1SetDateCombobox.get_active_text().split(' - ')[:-1])
            ...
            this_engine = self.ui.restore1SetDateCombobox.get_active_text().split(
                ' - ')[-1]
            if this_engine == 'rsync':
                backupName = '%s-%s-%s' % (_('Backup'),
                                           setConfig.getSetName(), date)
            else:
                # At the moment restore on Python 2.4 is not implemented; tarfile.extractall() doesn't exist.
                if sys.version_info[0] == 2 and sys.version_info[1] == 4:
                    self.displayError(self.ui.restore, _("Automated restore not supported"), _(
                        "This backup set cannot be automatically restored because restoration of Archive backups has not been implemented for systems with Python 2.4. Please see the user guide for details on manual restoration."))
                    return False
                backupName = '%s-%s-%s.%s' % (_('Backup'),
                                              setConfig.getSetName(), date, this_engine)
            if setConfig.get('Options', 'DestinationType') == 'remote (ssh)':
                options["RemoteHost"] = setConfig.get('Options', 'RemoteHost')
                options["RemoteUsername"] = setConfig.get(
                    'Options', 'RemoteUsername')
                options["RemotePassword"] = setConfig.get(
                    'Options', 'RemotePassword')
                options["RemotePort"] = setConfig.get('Options', 'RemotePort')
                remoteDestination = setConfig.get('Options', 'RemoteFolder')
                options["RemoteSource"] = os.path.join(
                    remoteDestination, backupName)
                # RemoteSource is transferred to Destination before restoring begins
                options["Source"] = os.path.join(
                    options["Destination"], backupName)
            else:
            ...
            options["Source"] = self.ui.restore1ArchiveEntry.get_text().decode(
                'utf-8')
        elif active == 2:  # local folder
            sourceType = 'local folder'
            options["Source"] = self.ui.restore1FolderEntry.get_text().decode(
                'utf-8')
            ...
            options["RemotePassword"] = self.ui.restore1PasswordEntry.get_text().encode(
                "base64")
            options["RemotePort"] = self.ui.restore1PortSpin.get_value_as_int()
            options["RemoteSource"] = self.ui.restore1PathEntry.get_text().decode(
                'utf-8')
            # RemoteSource is transferred to Destination before restoring begins
            options["Source"] = os.path.join(
                options["Destination"], os.path.basename(options["RemoteSource"]))
    ...
    def on_restoreStartButton_clicked(self, widget):
        ...
        if active == 0:  # Set backup
            if self.ui.restore1SetDateCombobox.get_active() == None:
                self.displayInfo(self.ui.restore, _(
                    'Missing information'), _('Please select a backup date.'))
                return False
            ...
            if self.ui.restore1SetDateCombobox.get_active_text() == _("No backups found"):
                self.displayError(self.ui.restore, _("No backups found"), _(
                    "The backup set '%s' does not have any stored backups yet! Choose another set, or alternatively choose a different restore source.") % setName)
                return False
            ...
            if not filename:
                self.displayInfo(self.ui.main, _('Restore source'), _(
                    'Please enter the location of the local archive.'))
                return False
            ...
            if not self.ui.restore1FolderEntry.get_text():
                self.displayInfo(self.ui.main, _('Restore source'), _(
                    'Please enter the location of the local folder.'))
                return False
            if not self.saveRestoreConfiguration(restoreConfig):
                return False
        elif active == 3:  # remote archive
    ...
    def on_restore1DestinationEntry_changed(self, widget):
        self._checkDestPerms(widget.get_text().decode(
            'utf-8'), self.ui.restore1DestinationPermissionImage)
    ...
    def _checkExistingOrNone(self, name, parent, entry):
        ...
        if name == None or name == '':
            self.displayInfo(self.ui.backupset, _(
                "Invalid set name"), _("Please enter a set name."))
            return False
        if name == 'temporary_config':
            self.displayInfo(self.ui.backupset, _("Invalid set name"), _(
                "'temporary_name' is a reserved name. Please choose another name for the set."))
            return False
    ...
    def restoreSetSettingsToUI(self, setConf):
        ...
        if t.strip() == 'False':  # easy config used
            ...
            # list isn't long enough - something's gone horrible wrong.
            except IndexError:
        ...
        if t == 'local':
            ...
            raise fwbackups.fwbackupsError(
                _('Unknown destination type `%s\'' % t))
        self.ui.backupset2HostEntry.set_text(
            setConf.get('Options', 'RemoteHost'))
        self.ui.backupset2PortSpin.set_value(
            float(setConf.get('Options', 'RemotePort') or 22))
        self.ui.backupset2UsernameEntry.set_text(
            setConf.get('Options', 'RemoteUsername'))
        self.ui.backupset2PasswordEntry.set_text(
            setConf.get('Options', 'RemotePassword').decode('base64'))
        self.ui.backupset2RemoteFolderEntry.set_text(
            setConf.get('Options', 'RemoteFolder'))
        ...
        self.ui.backupset4OldToKeepSpin.set_value(
            float(setConf.get('Options', 'OldToKeep')))
        # Advanced entries
        self.ui.backupset5NiceScale.set_value(
            float(setConf.get('Options', 'Nice')))
        self.ui.backupset5CommandBeforeEntry.set_text(
            setConf.get('Options', 'CommandBefore'))
        self.ui.backupset5CommandAfterEntry.set_text(
            setConf.get('Options', 'CommandAfter'))
        self.ui.backupset5ExcludesTextview.get_buffer().set_text(
            setConf.get('Options', 'Excludes'))
    ...
    def saveSetConfiguration(self, setConf):
        ...
        options["RemotePassword"] = self.ui.backupset2PasswordEntry.get_text().encode(
            'base64')
        options["RemotePort"] = self.ui.backupset2PortSpin.get_value_as_int()
        options["RemoteFolder"] = self.ui.backupset2RemoteFolderEntry.get_text().decode(
            'utf-8')
        options["Destination"] = self.ui.backupset2LocalFolderEntry.get_text().decode(
            'utf-8')
        # Save options
        options["Enabled"] = int(self.ui.backupset4EnableCheck.get_active())
        options["Recursive"] = int(
            self.ui.backupset4RecursiveCheck.get_active())
        options["PkgListsToFile"] = int(
            self.ui.backupset4PkgListsToFileCheck.get_active())
        options["DiskInfoToFile"] = int(
            self.ui.backupset4DiskInfoToFileCheck.get_active())
        options["BackupHidden"] = int(
            self.ui.backupset4BackupHiddenCheck.get_active())
        options["Sparse"] = int(self.ui.backupset4SparseCheck.get_active())
        options["FollowLinks"] = int(
            self.ui.backupset4FollowLinksCheck.get_active())
        options["Incremental"] = int(
            self.ui.backupset4IncrementalCheck.get_active())
        if self.ui.backupset4EngineRadio1.get_active():
            engine = 'tar'
        ...
        options["CommandAfter"] = self.ui.backupset5CommandAfterEntry.get_text().decode(
            'utf-8')
        ...
        options["Excludes"] = self.ui.backupset5ExcludesTextview.get_buffer().get_text(
            start, end)
    ...
    def main2IconviewSetup(self):
        ...
        self.main2BackupProgress = widgets.ProgressBar(
            self.ui.main2BackupProgress)
        self.main3TestSettingsProgress = widgets.ProgressBar(
            self.ui.main3TestSettingsProgress)
        self.backupset2TestSettingsProgress = widgets.ProgressBar(
            self.ui.backupset2TestSettingsProgress)
        self.restore1TestSettingsProgress = widgets.ProgressBar(
            self.ui.restore1TestSettingsProgress)
        self.backupset1PathView = widgets.PathView(
            self.ui.backupset1PathsTreeview, self.statusbar, self.ui, self.ui.backupset)
        self.ui.main2Iconview.set_model(
            gtk.ListStore(gobject.TYPE_STRING, gtk.gdk.Pixbuf))
        # Setup Iconview
        ...
        self.icon = self.ui.backupset.render_icon(
            gtk.STOCK_DIRECTORY, gtk.ICON_SIZE_DIALOG)
        # Keep things clean...
        self.main2IconviewRefresh()

    def main3TabSetup(self):
        """Setup the OneTime tab for use"""
        self.main3BackupProgress = widgets.ProgressBar(
            self.ui.main3BackupProgress)
        self.main3PathView = widgets.PathView(
            self.ui.main3Treeview, self.statusbar, self.ui, self.ui.main)
        # Keep things clean...
        self.main3Refresh()

    def restoreSetup(self):
        """Setup the OneTime tab for use"""
        self.restore2RestorationProgress = widgets.ProgressBar(
            self.ui.restore2RestorationProgress)
        ...
        for root, dirs, files in os.walk(SETLOC):
            ...
            for name in files:
                if name.endswith('.conf') and not name.startswith('.conf') and not name == 'temporary_config.conf':
                    self.ui.main2Iconview.get_model().append(
                        [name.split('.conf')[0], self.icon])
                    self.ui.restore1SetNameCombobox.get_model().append(
                        [name.split('.conf')[0]])
                    loaded_count += 1
                else:
                    self.logger.logmsg('WARNING', _(
                        'Refusing to parse file `%s\': configuration files must end in `.conf\'') % name)
    ...
    def on_backupsetApplyButton_clicked(self, widget):
            ...
            if name != newName:
                ...
                if os.path.isfile(namepath):
                    try:
                        os.remove(namepath)
                        self.logger.logmsg('DEBUG', _(
                            "Renaming set `%(a)s' to `%(b)s" % {'a': name, 'b': newName}))
                    except IOError as error:
                        self.logger.logmsg('DEBUG', _("Renaming set `%(a)s' to `%(b)s failed: %(c)s" % {
                                           'a': name, 'b': newName, 'c': error}))
                        self.displayInfo(self.ui.backupset, _("Could not rename set '%(a)s' to '%(b)s'") % {
                                         'a': name, 'b': newName}, str(error))
        ...
        try:
            ...
            self.displayError(self.ui.backupset, _("Could not save backup schedule"), _(
                "Your set configuration changes were saved, but fwbackups was unable to save the backup scheduling information to the crontab. If you are using manual times entry, please verify that all five fields are using the correct syntax."))
    ...
    def startSetBackup(self):
        ...
        def updateProgress(self):
            ...
            if status not in [backup.STATUS_INITIALIZING, backup.STATUS_EXECING_USER_COMMAND, backup.STATUS_CLEANING_OLD]:
                self.main2BackupProgress.set_fraction(
                    float(current - 1)/float(total))
            # There is a current filename
            if status == backup.STATUS_BACKING_UP and currentName not in [None, '', '\n']:
                self.main2BackupProgress.set_text(_('[%(a)i/%(b)i] Backing up: %(c)s') % {
                                                  'a': current, 'b': total, 'c': os.path.basename(currentName).strip('\n')})
            # There is no current filename
            elif status == backup.STATUS_BACKING_UP and not currentName:
                self.main2BackupProgress.set_text(
                    _('[%(a)i/%(b)i] Backing up files... Please wait') % {'a': current, 'b': total})
                ...
                self.main2BackupProgress.set_text(
                    _('Sending files to remote server'))
        ...
        try:
            ...
            self.displayWarning(self.ui.main, _('No set has been selected'), _(
                'You must select to initiate a manual set backup.'))
            return
        self.ui.mainControlNotebook.set_current_page(1)
        self.ui.BackupSetsRadioTool.set_active(True)
        prefs = config.PrefsConf()
        ...
        if int(prefs.get('Preferences', 'ShowNotifications')) == 1:
            self.trayNotify(_('Status'), _(
                'Starting a set backup operation of \'%(a)s\'' % {'a': name}), 5)
        self._toggleLocked(
            True, [self.ui.BackupSetsRadioTool, self.ui.backup_sets1])
        self.main2BackupProgress.startPulse()
        self.main2BackupProgress.set_text(_('Please wait...'))
        try:
            self.backupHandle = backup.SetBackupOperation(
                os.path.join(SETLOC, "%s.conf" % name))
            self.backupThread = fwbackups.runFuncAsThread(
                self.backupHandle.start)
            self.ui.main2CancelBackupButton.show()
            ...
            # thread returned
            self.logger.logmsg('DEBUG', _(
                'Thread returned with retval %s' % self.backupThread.retval))
            ...
            if self.backupThread.retval == -1:
                self.logger.logmsg('WARNING', _(
                    'There was an error while performing the backup!'))
                self.logger.logmsg('ERROR', self.backupThread.traceback)
        except Exception as error:
            self.operationInProgress = False
            self.setStatus(_('<span color="Red">Error</span>'))
            message = _(
                "An error occurred while initializing the backup operation: %s" % error)
            self.updateReturn = False
            self.main2BackupProgress.stopPulse()
            self.main2BackupProgress.set_text('')
            self.displayInfo(self.ui.main, _(
                "Error initializing backup"), message)
            self.logger.logmsg('DEBUG', message)
        ...
        if self.backupThread.retval == True:
            if int(prefs.get('Preferences', 'ShowNotifications')) == 1:
                self.trayNotify(_('Status'), _(
                    'Finished the automatic backup operation of set `%(a)s\'' % {'a': name}), 5)
            self.setStatus(
                _('<span color="dark green">Operation complete</span>'))
        elif self.backupThread.retval == -1 or self.backupThread.retval == False:  # error
            self.setStatus(_('<span color="Red">Error</span>'))
            if int(prefs.get('Preferences', 'ShowNotifications')) == 1:
                self.trayNotify(_('Status'), _(
                    'An error occured while performing the automatic backup operation of set `%(a)s\'' % {'a': name}), 5)
        else:
            if int(prefs.get('Preferences', 'ShowNotifications')) == 1:
                self.trayNotify(_('Status'), _(
                    'The automatic backup operation of set `%(a)s\' was cancelled' % {'a': name}), 5)
    ...
    def saveOneTimeConfiguration(self, oneTimeConf):
        ...
        options["RemotePassword"] = self.ui.main3PasswordEntry.get_text().encode(
            'base64')
        options["RemotePort"] = self.ui.main3PortSpin.get_value_as_int()
        options["RemoteFolder"] = self.ui.main3RemoteFolderEntry.get_text().decode(
            'utf-8')
        options["Destination"] = self.ui.main3LocalFolderEntry.get_text().decode(
            'utf-8')
        options["Recursive"] = int(self.ui.main3RecursiveCheck.get_active())
        options["PkgListsToFile"] = int(
            self.ui.main3PkgListsToFileCheck.get_active())
        options["DiskInfoToFile"] = int(
            self.ui.main3DiskInfoToFileCheck.get_active())
        options["BackupHidden"] = int(
            self.ui.main3BackupHiddenCheck.get_active())
        options["Sparse"] = int(self.ui.main3SparseCheck.get_active())
        options["FollowLinks"] = int(
            self.ui.main3FollowLinksCheck.get_active())
        ...
        options["Excludes"] = self.ui.main3ExcludesTextview.get_buffer().get_text(
            start, end)
    ...
    def startOneTimeBackup(self):
        ...
        def updateProgress(self):
            ...
            if self.updateReturn:
                ...
                return
            if status not in [backup.STATUS_INITIALIZING, backup.STATUS_EXECING_USER_COMMAND, backup.STATUS_CLEANING_OLD]:
                self.main3BackupProgress.set_fraction(
                    float(current - 1)/float(total))
            # There is a current filename
            if status == backup.STATUS_BACKING_UP and currentName not in [None, '', '\n']:
                self.main3BackupProgress.set_text(_('[%(a)i/%(b)i] Backing up: %(c)s') % {
                                                  'a': current, 'b': total, 'c': os.path.basename(currentName).strip('\n')})
            # There is no current filename
            elif status == backup.STATUS_BACKING_UP and not currentName:
        ...
        if int(prefs.get('Preferences', 'ShowNotifications')) == 1:
            self.trayNotify(_('Status'), _(
                'Starting a one-time backup operation'))
        self._toggleLocked(
            True, [self.ui.OneTimeRadioTool, self.ui.one_time_backup1])
        self.main3BackupProgress.startPulse()
        self.main3BackupProgress.set_text(_('Please wait...'))
        try:
            self.backupHandle = backup.OneTimeBackupOperation(
                ONETIMELOC, self.logger)
            self.backupThread = fwbackups.runFuncAsThread(
                self.backupHandle.start)
            self.setStatus(_('Working'))
            self.updateReturn = True
        ...
        if self.backupThread.retval == True:
            if int(prefs.get('Preferences', 'ShowNotifications')) == 1:
                self.trayNotify(_('Status'), _(
                    'Finished the one-time backup operation'))
            self.setStatus(
                _('<span color="dark green">Operation complete</span>'))
        elif self.backupThread.retval == -1 or self.backupThread.retval == False:  # error
            self.setStatus(_('<span color="Red">Error</span>'))
            if int(prefs.get('Preferences', 'ShowNotifications')) == 1:
                self.trayNotify(_('Status'), _(
                    'An error occured while performing the one-time backup operation'), 5)
            # just incase we have leftover stuff running
            self.backupHandle.cancelOperation()
        else:
            if int(prefs.get('Preferences', 'ShowNotifications')) == 1:
                self.trayNotify(_('Status'), _(
                    'The one-time backup operation was cancelled'))
            self.setStatus(_('<span color="red">Operation cancelled</span>'))
    ...
    def startRestore(self):
        ...
        def updateProgress(self):
            ...
            if status == restore.STATUS_RECEIVING_FROM_REMOTE:  # no "current file" yet
                self.restore2RestorationProgress.set_text(
                    _('Receiving files from remote server'))
            elif status == restore.STATUS_RESTORING:  # we have a 'current file'
                self.restore2RestorationProgress.set_text(
                    _('Restoring: %s') % os.path.basename(currentName))
            return self.updateReturn
        ...
        try:
            self.restoreHandle = restore.RestoreOperation(
                RESTORELOC, self.logger)
            self.restoreThread = fwbackups.runFuncAsThread(
                self.restoreHandle.start)
            ...
            # thread returned
            self.logger.logmsg('DEBUG', _(
                'Thread returned with retval %s' % self.restoreThread.retval))
            if self.restoreThread.retval == -1:
                self.logger.logmsg('WARNING', _(
                    'There was an error while performing the restore! Enable debug messages for more information.'))
            ...
            message = _(
                "An error occurred while initializing the restore operation: %s" % error)
            self.displayInfo(self.ui.main, _(
                "Error initializing restore operation"), message)
            self.logger.logmsg('DEBUG', message)
        ...
        if self.restoreThread.retval == True:
            ...
            if int(prefs.get('Preferences', 'ShowNotifications')) == 1:
                self.trayNotify(_('Status'), _(
                    'An error occured while performing the restore operation'), 5)
            # just incase we have leftover stuff running
            self.restoreHandle.cancelOperation()
        else:
            if int(prefs.get('Preferences', 'ShowNotifications')) == 1:
...
if __name__ == "__main__":
    ...
    parser = OptionParser(
        prog="fwbackups", version="fwbackups %s" % fwbackups.__version__)
    # Customize our help text for the pre-created options
    parser.get_option(
        "--version").help = _("Show this program's version and exit")
    parser.get_option("--help").help = _("Show this help message and exit")
    # Add custom options
    parser.add_option("-v", "--verbose", action="count", dest="verbose", default=0,
                      help=_("print log messages to the terminal. Use this option twice"))
    parser.add_option("--start-minimized", action="store_true", dest="start_minimized",
                      default=False, help=_("start GUI minimized in the system tray"))
