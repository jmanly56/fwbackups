...
from fwbackups.const import LOC, SETLOC, LINUX, DARWIN, USERHOME, INSTALL_DIR, MSWINDOWS, CRON_SIGNATURE, PREFSLOC
    ...
    for directory in [LOC, SETLOC]:
        if not os.path.exists(encode(directory)):
            try:
                ...
                raise ConfigError(
                    _("Could not create configuration folder `%s':" % error))
                sys.exit(1)
        # Passwords that are base64-encoded are stored,
        # so make sure they are secure
        if LINUX or DARWIN:
            if os.stat(encode(LOC)).st_mode != 16832:
                os.chmod(encode(LOC), 0o700)
        if not fwbackups.CheckPerms(directory):
            raise ConfigError(
                _("You do not have read and\
                        write permissions on folder `%s'.") % directory)
            sys.exit(1)


class ConfigFile(ConfigParser.ConfigParser):
    """A more sane implementation of the ConfigParser objects. It commits any
       changes to files immediately, and ensures that write() is not called
       more than once (avoiding duplication of the config file's contents).
       Additionally, read() is called after each write action so what is
       returned by the ConfigParser functions is exactly what is in the file.

       If create is True and the configuration file does not exist,
    ...
    def generateDict(self, sections=None):
        """Dumps the configuration settings into a multi-level dictionary object.
           If sections are specified (as a list), only the dicts for those
           sections will be generated and returned."""
        config = {}
        if not sections:
    ...
    def importDict(self, dictobject):
        ...
        """
        # In the loops, the set() method from parent class ConfigParser should
        # be used to avoid doing many consecutive writes. A final write()
        # is performed at the end to commit the changes.
        for section in dictobject.keys():
            if type(dictobject[section]) != dict:
                raise ValueError(
                    _("Key %s of is not a dictionary of options") % section)
            else:
...
class BackupSetConf:
    ...
    def __init__(self, setPath, create=False):
        ...
        if not self.__config.sections() and create:
            ...
            # If required, makes changes to the configuration files of previous
            # versions
            self.__import()
    ...
    def __import(self):
        ...
        except Exception:  # old config which was non-case sensitive
            oldVersion = self.__config.get('General', 'version')
        # Only if it's a version mismatch should we import...
        # Skip further checks if the configuration file matches the current
        # version
        ...
        if oldVersion == '1.43' or oldVersion.startswith('1.43.0') or fromHereUp is True:
            fromHereUp = True
        ...
        if oldVersion == '1.43.1' or fromHereUp is True:
            fromHereUp = True
        ...
        if oldVersion in ['1.43.2beta3', '1.43.2'] or fromHereUp is True:
            fromHereUp = True
        ...
        if oldVersion in ['1.43.2rc2', '1.43.2rc3'] or fromHereUp is True:
            fromHereUp = True
        # Remote password was obuscated in 1.43.3rc1
        if oldVersion == '1.43.2' or fromHereUp is True:
            fromHereUp = True
            encoded = self.__config.get(
                'Options', 'RemotePassword').encode('base64')
        ...
        if oldVersion in ['1.43.3rc1', '1.43.3rc2'] or fromHereUp is True:
            fromHereUp = True
            if self.__config.has_option("Options", "sparce"):
                self.__config.remove_option("Options", "sparce")
        if oldVersion in ['1.43.3rc3', '1.43.3rc4', '1.43.3rc5', '1.43.3rc6', '1.43.3', '1.43.4'] or fromHereUp is True:
            fromHereUp = True
    ...
    def __validate(self, strictValidation=False):
        ...
        if sorted(config.keys()) != ["General", "Options", "Paths", "Times"]:
            raise ValidationError(
                _("Set '%s' failed to pass section validation") % self.getSetName())
        # Ensure the set configuration is really a set configuration file
        if self.__config.get("General", "Type") != "Set":
            raise ValidationError(
                _("'%(a)s' is not a set configuration file.") % self.setPath)
        # Check that all required values are present
        # Validate General section - remember the list is sorted
        if not sorted(config["General"].keys()) == ["Type", "Version"]:
            raise ValidationError(
                _("Configuration section 'General' in the set configuration '%s' failed to validate")
                % self.getSetName())  # noqa: E131
        # Validate Times section - remember the list is sorted
        if not sorted(config["Times"].keys()) == ["Custom", "Entry"]:
            raise ValidationError(
                _("Configuration section 'Times' in the set configuration '%s' failed to validate") % self.getSetName())
        ...
        for option in sorted(config["Options"].keys()):
            if option not in validOptions:
                ...
                if strictValidation:
                    raise ValidationError(_("Unknown option key '%(a)s' present in set configuration '%(b)s'") % {
                                          'a': option, 'b': self.getSetName()})
            else:
                validOptions.remove(option)
        # If there are any option names left in validOptions, those were missing
        if validOptions:
            raise ValidationError(_("Configuration section 'Options' in set configuration '%(a)s' \
                    failed to validate: Missing options '%(b)s'") % {
                                  'a': self.getSetName(), 'b': ', '.join(validOptions)})
    ...
    def getCronLineText(self):
        ...
        try:
            ...
            if MSWINDOWS:  # needs an abs path because pycron=system user
                entry.append('"%s" "%s\\fwbackups-run.py" -l "%s"' % (sys.executable,
                                                                      INSTALL_DIR,
                                                                      fwbackups.escapeQuotes(self.getSetName(), 2)))
            else:
                entry.append('fwbackups-run -l \'%s\'' %
                             fwbackups.escapeQuotes(self.getSetName(), 1))
    ...
    def save(self, paths, options, times, mergeDefaults=False):
        ...
        try:  # Attempt to validate the file
            self.__validate()
        except Exception:  # Restore original backup configuration and raise an error
            os.remove(encode(self.setPath))
            self.__config = ConfigFile(self.setPath, True)
...
class OneTimeConf:
    ...
    def __validate(self, strictValidation=False):
        ...
        if sorted(config.keys()) != ["General", "Options", "Paths"]:
            raise ValidationError(
                _("One-time configuration failed to pass section validation"))
        # Ensure the set configuration is really a set configuration file
        if self.__config.get("General", "Type") != "OneTime":
            raise ValidationError(
                _("'%(a)s' is not a one-time configuration file.") % self.onetPath)
        # Check that all required values are present
        # Validate General section - remember the list is sorted
        if not sorted(config["General"].keys()) == ["Type", "Version"]:
            raise ValidationError(
                _("Configuration section 'General' in the one-time configuration file failed to validate"))
        # Validate Options section - remember the list is sorted
        ...
        for option in sorted(config["Options"].keys()):
            if option not in validOptions:
                ...
                if strictValidation:
                    raise ValidationError(
                        _("Unknown option key '%s' present in one-time backup configuration file") % option)
            else:
                validOptions.remove(option)
        # If there are any option names left in validOptions, those were missing
        if validOptions:
            raise ValidationError(
                _("Configuration section 'Options' in the one-time backup configuration file \
                        failed to validate: Missing options '%s'") % ', '.join(validOptions))
...
class RestoreConf:
    ...
    def __validate(self, strictValidation=False):
        ...
        if sorted(config.keys()) != ["General", "Options"]:
            raise ValidationError(
                _("Restore configuration failed to pass section validation"))
        # Ensure the set configuration is really a set configuration file
        if self.__config.get("General", "Type") != "Restore":
            raise ValidationError(
                _("'%(a)s' is not a restore configuration file.") % self.restorePath)
        # Check that all required values are present
        # Validate General section - remember the list is sorted
        if not sorted(config["General"].keys()) == ["Type", "Version"]:
            raise ValidationError(
                _("Configuration section 'General' in the restore configuration file failed to validate"))
        # Validate Options section - remember the list is sorted
        ...
        for option in sorted(config["Options"].keys()):
            if option not in validOptions:
                ...
                if strictValidation:
                    raise ValidationError(
                        _("Unknown option key '%s' present in restore configuration file") % option)
            else:
                validOptions.remove(option)
        # If there are any option names left in validOptions, those were missing
        if validOptions:
            raise ValidationError(
                _("Configuration section 'Options' in the restore configuration \
                        file failed to validate: Missing options '%s'") % ', '.join(validOptions))
...
class PrefsConf:
    ...
    def __validate(self, strictValidation=False):
        ...
        if sorted(config.keys()) != ["General", "Preferences"]:
            raise ValidationError(
                _("Preferences failed to pass section validation"))
        # Ensure the set configuration is really a set configuration file
        if self.__config.get("General", "Type") != "Preferences":
            raise ValidationError(
                _("'%(a)s' is not a fwbackups preferences file.") % PREFSLOC)
        # Check that all required values are present
        # Validate General section - remember the list is sorted
        if not sorted(config["General"].keys()) == ["Type", "Version"]:
            raise ValidationError(
                _("Configuration section 'General' in the preferences file failed to validate"))
        # Validate Options section - remember the list is sorted
        ...
        for option in sorted(config["Preferences"].keys()):
            if option not in validOptions:
                ...
                if strictValidation:
                    raise ValidationError(
                        _("Unknown option key '%s' present in preferences file") % option)
            else:
                validOptions.remove(option)
        # If there are any option names left in validOptions, those were missing
        if validOptions:
            raise ValidationError(
                _("Configuration section 'Options' in the preferences file \
                        failed to validate: Missing options '%s'") % ', '.join(validOptions))
    ...
    def __import(self):
        ...
        try:
            oldVersion = self.__config.get('General', 'Version')
        except Exception:  # old config from beta3
            oldVersion = self.__config.get('General', 'version')
        # only if it's a version mismatch should we import
        ...
        if oldVersion == '1.43.0' or fromHereUp is True:
            fromHereUp = True
        ...
        if oldVersion == '1.43.1' or fromHereUp is True:
            ...
            self.__config.set('Preferences', 'pycronloc',
                              'C:\\Program Files\\pycron')
            self.__config.set(
                'Preferences', 'dontshowme_netconnectunresponsive', 0)
        # just do the other stuff
        if oldVersion == '1.43.2beta1' or fromHereUp is True:
            fromHereUp = True
        # There was no 1.43.2beta2
        if oldVersion == '1.43.2beta3' or fromHereUp is True:
            fromHereUp = True
            ...
            for option in ['ShowTrayIcon', 'MinimizeTrayClose', 'StartMinimized',
                ...
                self.__config.set('Preferences', option, self.__config.get(
                    'Preferences', option.lower()))
                self.remove_option('Preferences', option.lower())
            # --
            self.remove_option(
                'Preferences', 'DontShowMe_NetConnectUnresponsive')
        # just do stuff below
        if oldVersion in ['1.43.2rc1', '1.43.2rc2', '1.43.2rc3', '1.43.2', '1.43.3rc1',
                          '1.43.3rc2', '1.43.3rc3', '1.43.3rc4'] or fromHereUp is True:
            fromHereUp = True
        if oldVersion == '1.43.3rc5' or fromHereUp is True:
            fromHereUp = True
            ...
            except Exception:  # write backup
                cron.write(crontabLines)
                raise
                raise ConfigError(_("Unable to clean user crontab!"))
        if oldVersion in ['1.43.3rc6', '1.43.3', '1.43.4'] or fromHereUp is True:
            fromHereUp = True
