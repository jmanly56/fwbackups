...
from fwbackups.const import MSWINDOWS, EXIT_STATUS_OK
...
class BackupOperation(operations.Common):
        ...
        for path in os.environ['PATH'].split(':'):
            if os.path.exists(os.path.join(path, 'rpm')) and "rpm" not in pkgListFiles:
                fd, path = tempfile.mkstemp(
                    suffix='.txt', prefix="%s - tmp" % _('rpm - Package list'))
                fh = os.fdopen(fd, "w")
                ...
                if pyrpm:
                    ...
                    retval, stdout, stderr = fwbackups.execute(
                        'rpm -qa', env=self.environment, shell=True, stdoutfd=fh)
                pkgListFiles['rpm'] = path
                fh.close()
            if os.path.exists(os.path.join(path, 'pacman')) and "pacman" not in pkgListFiles:
                fd, path = tempfile.mkstemp(
                    suffix='.txt', prefix="%s - tmp" % _('Pacman - Package list'))
                fh = os.fdopen(fd, 'w')
                retval, stdout, stderr = fwbackups.execute(
                    'pacman -Qq', env=self.environment, shell=True, stdoutfd=fh)
                pkgListFiles['pacman'] = path
                fh.close()
            if os.path.exists(os.path.join(path, 'dpkg')) and "dpkg" not in pkgListFiles:
                fd, path = tempfile.mkstemp(
                    suffix='.txt', prefix="%s - tmp" % _('dpkg - Package list'))
                fh = os.fdopen(fd, 'w')
                retval, stdout, stderr = fwbackups.execute(
                    'dpkg -l', env=self.environment, shell=True, stdoutfd=fh)
    ...
    def createDiskInfo(self):
        ...
        fd, path = tempfile.mkstemp(
            suffix='.txt', prefix="%s - tmp" % _('Disk Information'))
        fh = os.fdopen(fd, 'w')
        retval, stdout, stderr = fwbackups.execute(
            'fdisk -l', env=self.environment, shell=True, stdoutfd=fh)
        fh.close()
        return path
    ...
    def parseCommand(self):
        ...
        if self.options['DestinationType'] == 'remote (ssh)':
            ...
            tempDir = prefs.get(
                'Preferences', 'TempDir') or tempfile.gettempdir()
            self.dest = os.path.join(tempDir, os.path.split(
                fwbackups.escapeQuotes(self.dest, 1))[1])
    ...
    def checkRemoteServer(self):
        ...
        self.logger.logmsg('DEBUG', _(
            'Attempting to connect to server %s...') % self.options['RemoteHost'])
        ...
        if thread.retval is True:
            ...
            self.logger.logmsg('ERROR', _('A connection to the server could not be established.\n' +
                                          'Error %(a)s: %(b)s' % {'a': type(thread.exception),
                                              'b': str(thread.exception)} +  # noqa: E128
    ...
    def backupPaths(self, paths, command):
        ...
        if self.options['Engine'] == 'tar':
            if MSWINDOWS:
                self.logger.logmsg('INFO', _(
                    'Using %s on Windows: Cancel function will only take effect \
                            after a path has been completed.' % self.options['Engine']))
                import tarfile
                ...
                for i in paths:
                    ...
                    self._current += 1
                    self.logger.logmsg('DEBUG', _(
                        'Backing up path %(a)i/%(b)i: %(c)s' % {'a': self._current, 'b': self._total, 'c': i}))
                ...
                for i in paths:
                    ...
                    self.ifCancel()
                    self._current += 1
                    self.logger.logmsg('DEBUG', _("Running command: nice -n %(a)i %(b)s '%(c)s'" % {
                                       'a': self.options['Nice'], 'b': command, 'c': i}))
                    sub = fwbackups.executeSub(
                        "nice -n %i %s '%s'" % (self.options['Nice'], command, i), env=self.environment, shell=True)
                    self.pids.append(sub.pid)
                    self.logger.logmsg('DEBUG', _(
                        'Starting subprocess with PID %s') % sub.pid)
                    # track stdout
                    errors = []
                    # use nonblocking I/O
                    ...
                    retval = sub.poll()
                    self.logger.logmsg('DEBUG', _(
                        'Subprocess with PID %(a)s exited with status %(b)s' % {'a': sub.pid, 'b': retval}))
                    # Something wrong?
                    if retval != EXIT_STATUS_OK and retval != 2:
                        wasAnError = True
                        self.logger.logmsg(
                            'ERROR', 'An error occurred while backing up path \'%s\'.\nPlease check the error output below to \
                                    determine if any files are incomplete or missing.' % str(i))
                        self.logger.logmsg('ERROR', _('Process exited with status %(a)s. Errors: %(b)s' % {
                                           'a': retval, 'b': ''.join(errors)}))

        elif self.options['Engine'] == 'tar.gz':
            self._total = 1
            if MSWINDOWS:
                self.logger.logmsg('INFO', _(
                    'Using %s on Windows: Cancel function will only take\
                            effect after a path has been completed.' % self.options['Engine']))
                import tarfile
                ...
                for i in paths:
                    ...
                    fh.add(i, recursive=self.options['Recursive'])
                    self.logger.logmsg('DEBUG', _(
                        'Adding path `%s\' to the archive' % i))
                fh.close()
            else:  # not MSWINDOWS
                self._current = 1
                ...
                self.logger.logmsg('INFO', _(
                    'Using %s: Must backup all paths at once - Progress notification \
                            will be disabled.' % self.options['Engine']))
                self.logger.logmsg('DEBUG', _('Backing up path %(a)i/%(b)i: %(c)s') %
                                   {'a': self._current, 'b': self._total, 'c': i.replace("'", '')})
                self.logger.logmsg('DEBUG', _("Running command: nice -n %(a)i %(b)s %(c)s" % {
                                   'a': self.options['Nice'], 'b': command, 'c': i}))
                ...
                if retval != EXIT_STATUS_OK and retval != 2:
                    ...
                    self.logger.logmsg(
                        'ERROR', 'An error occurred while backing up path \'%s\'.\nPlease check the error \
                                output below to determine if any files are incomplete or missing.' % str(i))
                    self.logger.logmsg('ERROR', _('Process exited with status %(a)s. Errors: %(b)s' % {
                                       'a': retval, 'b': ''.join(errors)}))
            ...
            if MSWINDOWS:
                ...
                i = "'%s'" % "' '".join(escapedPaths)
                self.logger.logmsg('INFO', _(
                    'Using %s: Must backup all paths at once - Progress notification will \
                            be disabled.' % self.options['Engine']))
                self.logger.logmsg('DEBUG', _('Backing up path %(a)i/%(b)i: %(c)s') %
                                   {'a': self._current, 'b': self._total, 'c': i})
                self.logger.logmsg('DEBUG', _("Running command: nice -n %(a)i %(b)s %(c)s" % {
                                   'a': self.options['Nice'], 'b': command, 'c': i}))
                # Don't wrap i in quotes; we did this above already when mering the paths
                sub = fwbackups.executeSub(
            ...
            if self.options['DestinationType'] == 'remote (ssh)':
                    ...
                    for i in paths:
                        ...
                        if not os.path.exists(encode(i)):
                            self.logger.logmsg('WARNING', _(
                                "Path %s is missing or cannot be read and will be excluded from the backup.") % i)
                        sftp.put(sftpClient, encode(i), encode(os.path.normpath(
                            self.options['RemoteFolder']+os.sep+os.path.basename(
                                self.dest)+os.sep+os.path.dirname(i))),
                                symlinks=not self.options['FollowLinks'],
                                excludes=encode(self.options['Excludes'].split('\n')))
                sftpClient.close()
                client.close()
                ...
                for i in paths:
                    ...
                    if MSWINDOWS:
                        ...
                        self.logger.logmsg('DEBUG', _(
                            'Backing up path %(a)i/%(b)i: %(c)s' % {'a': self._current, 'b': self._total, 'c': i}))
                        shutil_modded.copytree_fullpaths(
                            encode(i), encode(self.dest))
                    else:  # not MSWINDOWS; UNIX/OS X can call rsync binary
                        i = fwbackups.escapeQuotes(i, 1)
                        self.logger.logmsg('DEBUG', _("Running command: nice -n %(a)i %(b)s %(c)s '%(d)s'" % {
                                           'a': self.options['Nice'], 'b': command, 'c': i, 'd': fwbackups.escapeQuotes(
                                               self.dest, 1)}))
                        sub = fwbackups.executeSub("nice -n %i %s '%s' '%s'" % (
                            self.options['Nice'], command, i, fwbackups.escapeQuotes(self.dest, 1)),
                        ...
                        # use nonblocking I/O
                        fl = fcntl.fcntl(sub.stderr, fcntl.F_GETFL)
                        fcntl.fcntl(sub.stderr, fcntl.F_SETFL,
                                    fl | os.O_NONBLOCK)
                        while sub.poll() in ["", None]:
                            time.sleep(0.01)
                            try:
        ...
        if self.options['Engine'].startswith('tar') and self.options['DestinationType'] == 'remote (ssh)' \
            ...
            client, sftpClient = sftp.connect(
                ...
                self.options['RemotePassword'], self.options['RemotePort'])
            try:
                sftp.putFile(sftpClient, self.dest,
                             self.options['RemoteFolder'])
                os.remove(self.dest)
            except Exception:
                import sys
                ...
                wasAnError = True
                self.logger.logmsg('DEBUG', _(
                    'Could not send file(s) or folder to server:'))
                (etype, value, tb) = sys.exc_info()
                self.logger.logmsg('DEBUG', ''.join(
    ...
    def start(self):
        ...
        raise NotImplementedError(
            _('This function is designed to be subclassed'))
...
class OneTimeBackupOperation(BackupOperation):
    def __init__(self, onetPath, logger=None):
        ...
        self.dest = os.path.join(
            self.options['Destination'], "%s-%s-%s" % (_('Backup'), _('OneTime'), date))
    ...
    def start(self):
        ...
        # check if server settings are OK
        if self.options['DestinationType'] == 'remote (ssh)':
        ...
        if not (self.options['Engine'] == 'rsync' and self.options['Incremental'] and
            ...
            if os.path.exists(encode(self.dest)):
                self.logger.logmsg('WARNING', _(
                    '`%s\' exists and will be overwritten.') % self.dest)
                shutil_modded.rmtree(encode(self.dest), onerror=self.onError)
        self.ifCancel()

        command = self.parseCommand()
        self.addListFilesToBackup(
            pkgListfiles, command, self.options['Engine'], paths)
...
class SetBackupOperation(BackupOperation):
    ...
    def __init__(self, setPath, logger=None, forceRun=False):
        ...
        self.dest = os.path.join(
            self.options['Destination'], u"%s-%s-%s" % (_('Backup'), self.config.getSetName(), self.date))
    ...
    def tokens_replace(self, text, date, successful=None):
        ...
        for token, sub in tokens.items():
            ...
            text = re.sub(r"([^\\]+?)\[(%s)\]" %
                          token, replace_match_escape, text)
    ...
    def execute_user_command(self, cmd_type, command):
        ...
        retval = sub.poll()
        # Something wrong?
        if retval is not EXIT_STATUS_OK:
            self.logger.logmsg('ERROR', _(
                'Command returned with a non-zero exit status!'))
            self.logger.logmsg('ERROR', _('Process exited with status %(a)s. Errors: %(b)s' % {
                               'a': retval, 'b': ''.join(errors)}))
    ...
    def removeOldBackups(self):
        ...
        listing = [i.decode('utf-8') for i in listing]
        listing.sort()
        oldbackups = []
        for i in listing:
        ...
        if self.options['DestinationType'] == 'remote (ssh)':
            for i in oldbackups[self.options['OldToKeep']:]:
                ...
                self.logger.logmsg('DEBUG', _('Removing old backup `%(a)s\' on %(b)s') % {
                                   'a': remoteBackup, 'b': self.options['RemoteHost']})
            ...
            if self.options['Engine'] == 'rsync' and self.options['Incremental'] and oldbackups:
                ...
                if not oldIncrementalBackup.endswith('.tar') and not oldIncrementalBackup.endswith('.tar.gz') and \
                    ...
                    self.logger.logmsg('DEBUG', _('Moving  `%s\' to `%s\'') % (
                        oldIncrementalBackup, self.dest))
                    shutil_modded.move(
                        encode(oldIncrementalBackup), encode(self.dest))
                else:  # source = is not a rsync backup - remove it and start fresh
                    self.logger.logmsg('DEBUG', _(
                        '`%s\' is not an rsync backup - removing.') % oldIncrementalBackup)
                    shutil_modded.rmtree(
                        encode(oldIncrementalBackup), onerror=self.onError)
            else:
                for i in oldbackups[self.options['OldToKeep']:]:
                    self.logger.logmsg('DEBUG', _(
                        'Removing old backup `%s\'') % i)
    ...
    def start(self):
        ...
        self.logger.logmsg('INFO', _(
            'Starting automatic backup operation of set `%s\'') % self.config.getSetName())

        if self.options["CommandBefore"]:
            self._status = STATUS_EXECING_USER_COMMAND
            # Find tokens and substitute them
            tokenized_command = self.tokens_replace(
                self.options["CommandBefore"], self.date)
            ...
            if not (self.options['Engine'] == 'rsync' and self.options['Incremental']) and \
                ...
                if not (self.options['Engine'] == 'rsync' and self.options['Incremental']) \
                    ...
                    self.logger.logmsg('WARNING', _(
                        '`%s\' exists and will be overwritten.') % self.dest)
                    shutil_modded.rmtree(
                        encode(self.dest), onerror=self.onError)
            self.ifCancel()
        ...
        if self.options["CommandAfter"]:
            ...
            # Find tokens and substitute them
            tokenized_command = self.tokens_replace(
                self.options["CommandAfter"], self.date, retval)
            self.execute_user_command(2, tokenized_command)

        # All done!
        self.logger.logmsg('INFO', _(
            "Finished automatic backup operation of set '%s'") % self.config.getSetName())
        return retval
